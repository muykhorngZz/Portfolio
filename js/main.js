...
(function () {
  ...
  /* ---------- shared certificate/PDF tile ----------
     The PDF equivalent of the image placeholders: <object> renders a
     preview of the file, and if the path 404s the browser shows the
     fallback inside it instead — same "never a broken slot" behaviour
     as `onerror` gives the <img> tags. The preview ignores pointer
     events so a click lands on the surrounding link and opens the
     full document.                                                     */
  function pdfDoc(pdf) {
    if (!pdf) return "";
    ...
    if (!src) return "";
    const cap = esc((typeof pdf === "object" && pdf.caption)
      || "Add " + src.split("/").pop());
    return `
      <a class="doc" href="${esc(src)}" target="_blank" rel="noopener"
         aria-label="View PDF — opens in a new tab">
        <object class="doc-preview" type="application/pdf"
                data="${esc(src)}#toolbar=0&navpanes=0&scrollbar=0&view=FitH">
          <span class="doc-empty">${cap}</span>
        </object>
