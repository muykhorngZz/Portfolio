...
(function () {
  ...
  function pdfDoc(pdf) {
    if (!pdf) return "";
    // `pdf` is either a path string or { src } — a bare path is enough.
  ...
  /* Drops buttons whose file isn't there yet, so a visitor never sees a
     link to a document that doesn't exist. A HEAD request keeps it cheap —
     the PDF itself is never downloaded. Anything other than a clear 404
     (offline, opened over file://, a server that rejects HEAD) leaves the
     button alone: hiding a certificate that does exist is the worse
     failure of the two.                                                  */
  function markMissingPdfs() {
    if (!window.fetch || location.protocol === "file:") return;
    const seen = new Map();   // one request per path, not per button
    $$("a.doc").forEach(a => {
      ...
      seen.get(src).then(missing => { if (missing) a.remove(); });
    });
  }
