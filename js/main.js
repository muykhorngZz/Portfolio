...
(function () {
  ...
  /* ---------- shared certificate/PDF tile ----------
     Deliberately shows a closed cover, never the document itself —
     certificates carry personal details, so nothing is readable until
     the visitor chooses to open it. Missing files are found by asking
     the server (see markMissingPdfs) rather than by embedding them.   */
  function pdfDoc(pdf) {
    if (!pdf) return "";
    // `pdf` is either a path string or { src, caption } when the default
    // "Add <filename>" placeholder text isn't what you want.
    const src = typeof pdf === "string" ? pdf : pdf.src;
    if (!src) return "";
    const file = src.split("/").pop();
    const cap  = esc((typeof pdf === "object" && pdf.caption) || "Add " + file);
    return `
      <a class="doc" href="${esc(src)}" target="_blank" rel="noopener"
         data-missing-label="${cap}"
         aria-label="View PDF — opens in a new tab">
        <span class="doc-open">View PDF</span>
      </a>`;
  }

  /* Flips tiles whose file isn't there yet into a labelled placeholder.
     A HEAD request keeps it cheap — the PDF itself is never downloaded.
     Anything other than a clear 404 (offline, opened over file://, a
     server that rejects HEAD) leaves the tile alone: a working link
     shown as "missing" is a worse failure than the reverse.            */
  function markMissingPdfs() {
    if (!window.fetch || location.protocol === "file:") return;
    const seen = new Map();   // one request per path, not per tile
    $$("a.doc").forEach(a => {
      const src = a.getAttribute("href");
      if (!seen.has(src)) {
        seen.set(src, fetch(src, { method: "HEAD" })
          .then(r => r.status === 404)
          .catch(() => false));
      }
      seen.get(src).then(missing => {
        if (!missing) return;
        a.classList.add("is-empty");
        a.removeAttribute("href");          // nothing to open
        a.setAttribute("aria-hidden", "true");
        $(".doc-open", a).textContent = a.dataset.missingLabel;
      });
    });
  }

  /* ---------- HOME portrait rotation ---------- */
  const heroBox    = $("#heroPortrait");
  const heroPhotos = DATA.heroPhotos || [];

  if (heroBox && heroPhotos.length) {
    // All photos are stacked in the same cell and crossfaded, so the
    // surrounding layout never shifts as they change.
    heroBox.innerHTML = heroPhotos.map((ph, i) => `
      <figure class="portrait${i ? "" : " is-on"}" ${i ? 'aria-hidden="true"' : ""}>
        <img src="${esc(ph.src)}" alt="${esc(ph.alt || "")}"
             onerror="this.closest('figure').classList.add('is-placeholder')" />
        <figcaption>${esc(ph.caption || "")}</figcaption>
      </figure>`).join("");

    const shots = $$(".portrait", heroBox);

    if (shots.length > 1 && !matchMedia("(prefers-reduced-motion: reduce)").matches) {
      let current = 0;
      setInterval(() => {
        shots[current].classList.remove("is-on");
        shots[current].setAttribute("aria-hidden", "true");
        current = (current + 1) % shots.length;
        shots[current].classList.add("is-on");
        shots[current].removeAttribute("aria-hidden");
      }, DATA.heroInterval || 10000);
    }
  }
  ...
  // both sections are in the DOM by now, so check every tile at once
  markMissingPdfs();
  ...
  if (stripShots.length) {
    ...
    // The marker goes on the figure as well as the image: cleanup removes whole
    // figures, and an image-only marker leaves empty shells behind that still
    // occupy a flex slot and a gap.
    const tile = (ph, dup) => `
      <figure class="st-shot"${dup ? ' data-dup="1" aria-hidden="true"' : ""}>
    ...
    // Every photo must have finished loading (or failed) before we measure —
    // a pending image is 0px wide, and measuring then produces a short
    // set width, which shows up as a gap when the loop wraps.
    const originals = () => $$("img:not([data-dup])", strip);
    const allSettled = () => originals().every(i => i.complete);

    function layoutStrip() {
      if (!allSettled()) return;   // a load/error event will call us back

      strip.querySelectorAll(".st-shot[data-dup]").forEach(n => n.remove());
      ...
      /* Duration is derived from the measured width so the strip always
         ...
         there are or how wide they are. */
      const PX_PER_SEC = 53;

      /* Restart the animation rather than just updating the values. A running
         animation keeps the keyframe distance it started with, so a changed
         --shift would not take effect until the next cycle — leaving one
         visibly wrong lap. */
      strip.style.animation = "none";
      void strip.offsetWidth;                       // force reflow
      strip.style.setProperty("--shift", setWidth + "px");

      // an inline animation would beat the reduced-motion rule in the CSS,
      // so leave it off entirely for people who asked for less movement
      if (!matchMedia("(prefers-reduced-motion: reduce)").matches) {
        strip.style.animation =
          `strip-slide ${setWidth / PX_PER_SEC}s linear infinite`;
      }
    }
    ...
    let stripTimer;
    const relayout = () => { clearTimeout(stripTimer); stripTimer = setTimeout(layoutStrip, 90); };
    ...
    // catches anything the events above miss (fonts, late reflows)
    if (window.ResizeObserver) new ResizeObserver(relayout).observe(strip.parentElement);
    relayout();
  } else {
